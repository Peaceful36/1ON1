...
function Calendars() {
  ...
  const handleDelete = async (calendar_id) => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/calendars/${calendar_id}/delete`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      const updatedContacts = calendar.filter(calendar => calendar.id !== calendar_id);
      setCalendars(updatedContacts);
      fetchCalendars();
    } catch (error) {
  ...
  return (
    <>
    <Navbar />
    <h1 className="text-white text-7xl font-staatliches text-center my-5">Calendars</h1>

    {/* component */}
    <div className='flex items-center justify-center my-10'>
        <div className="flex w-full pl-5 pr-2 md:px-0 md:w-1/3 md:mx-10 rounded-3xl">
            <input className="w-full h-12 rounded-3xl px-4 py-1 text-gray-400 outline-none focus:outline-none" type="search" name="search" placeholder="Search..." />
        </div>
        <Link to="/addcalendar">
    ...
    <div className="relative flex flex-col justify-center md:py-6 mb-4">
        <div className="flex flex-col items-center mx-auto max-w-screen-lg">
            <div className="grid w-full gap-10 grid-cols-1 md:grid-cols-3 px-5 md:px-0">
                ...
                <Link to="/addcalendar" className="hidden hover:bg-gray-500 cursor-pointer w-full rounded-lg shadow-md md:flex flex-col transition-all overflow-hidden hover:shadow-2xl border-dotted border-2 border-gray-300">
                    <div className="my-auto p-6 flex flex-col items-center justify-center">
                        <div className="my-auto mx-auto p-6 flex items-center justify-center">
                            ...
                            <span className="ml-2 text-white font-staatliches text-lg">New Calendar</span>
                        </div>
                    </div>
                </Link>
                ...
                {calendar.map((cal, index) => (
                    <div key={index} className="bg-white w-full rounded-lg shadow-md flex flex-col transition-all overflow-hidden hover:shadow-2xl">
                        <div className="p-6">
                            ...
                            <h3 className="mb-4 font-semibold text-2xl font-staatliches"><Link to={`/calendar_view/${cal.id}`} className="custom-hover custom-blue font-staatliches hover:text-[#F44336]">{cal.title}</Link></h3>
                          ...
                          <Link
                            to="/editcalendar"
                            state={{title: cal.title, description: cal.description, id: cal.id}}
                            className="text-md mb-0 text-red-400 font-staatliches hover:underline"
                          >
                            EDIT
                            </Link>
                            <p className="text-md mb-0 ml-3 text-red-400 font-staatliches hover:underline" onClick={() => handleDelete(cal.id)}>
                                DELETE
                            </p>
                          </div>  
                        </div>
